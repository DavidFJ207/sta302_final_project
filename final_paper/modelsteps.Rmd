...
- Steps Taken:  
  ...
  - Merged datasets from 1997–2016 
  - Split the Data:  
...
- Tools Used: `ggplot2` for visualization 
- Key Insights:  
...
eda_training_data <- train_data %>%
  dplyr::select(any_of(c(response, predictors)))
...
The performance metrics demonstrate the effectiveness of the regression model on the test data. The Mean Squared Error (MSE) of 1.919877 reflects a small average error between the predicted and actual values, indicating high accuracy. The R-squared value of 0.993665 reveals that the model explains 99.37% of the variability in the test data, showcasing an excellent fit. These results confirm the model's strong predictive ability and validate its generalizability to unseen data.
\newpage  